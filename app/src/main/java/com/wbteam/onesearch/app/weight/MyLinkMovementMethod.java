...
public class MyLinkMovementMethod extends LinkMovementMethod {
	...
	private void a(TextView textview, Spannable span, ClickableSpan clickSpan) {
		if (clickSpan != null && a(span, clickSpan)) {
			span.setSpan(new BackgroundColorSpan(0), span.getSpanStart(clickSpan), span.getSpanEnd(clickSpan), 33);
	...
	public boolean onTouchEvent(TextView textview, Spannable spannable, MotionEvent event) {
		...
		if (i == 0) {
			...
			if (l2 != i3) {
				aclickablespan1 = (ClickableSpan[]) spannable.getSpans(l2, l2, ClickableSpan.class);
				if (aclickablespan1 != null && aclickablespan1.length != 0) {
					...
					if (clickablespan1 != null && a(spannable, clickablespan1)) {
						...
						spannable.setSpan(new BackgroundColorSpan(textview.getContext().getResources().getColor(R.color.timeline_clickable_text_highlighted_background)), spannable.getSpanStart(clickablespan1),
								spannable.getSpanEnd(clickablespan1), 33);
					}
				}
			} else {
				flag = true;
			...
			if (a == null) {
				...
				int j2 = layout1.getOffsetForHorizontal(layout1.getLineForVertical(k1), j1);
			...
			if (b == null) {
				...
				if (i2 != layout.getOffsetForHorizontal(l1, j1 + c)) {
					aclickablespan = (ClickableSpan[]) spannable.getSpans(i2, i2, ClickableSpan.class);
					if (aclickablespan != null && aclickablespan.length != 0) {
						...
						if (clickablespan != null && a(spannable, clickablespan)) {
							b = clickablespan;
							Selection.removeSelection(spannable);
							spannable.setSpan(new BackgroundColorSpan(textview.getContext().getResources().getColor(R.color.timeline_clickable_text_highlighted_background)), spannable.getSpanStart(clickablespan),
									spannable.getSpanEnd(clickablespan), 33);
						}
					}
				} else {
					flag = true;
