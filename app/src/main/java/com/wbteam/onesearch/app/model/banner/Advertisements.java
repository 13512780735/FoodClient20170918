...
public class Advertisements implements OnPageChangeListener {
    ...
    public Advertisements(Context context, LayoutInflater inflater, int timeDratioin) {
        this.context = context;
        this.inflater = inflater;
        this.timeDratioin = timeDratioin;
    }

    public View initView(final BizResultOfBanner advertiseArray) {
        ...
        for (int i = 0; i < advertiseArray.getBannerList().size(); i++) {
        ...
        if (advertiseArray.getBannerList().size() == 1) {
        ...
        task = new TimerTask() {
            @Override
            public void run() {
                int currentPage = count % advertiseArray.getBannerList().size();
                count++;
