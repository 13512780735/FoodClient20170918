package com.wbteam.app.base.adapter;
...
public class ListBaseAdapter<T extends Entity> extends BaseAdapter {
	...
	public ListBaseAdapter() {
		...
		_clickLookMore = R.string.click_look_more;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (position == getCount() - 1) {// 最后一条
			...
			if (getState() == STATE_LOAD_MORE || getState() == STATE_NO_MORE || state == STATE_EMPTY_ITEM || getState() == STATE_NETWORK_ERROR) {
				this.mFooterView = (RelativeLayout) LayoutInflater.from(parent.getContext()).inflate(R.layout.list_cell_footer, null);
				if (!loadMoreHasBg()) {
					mFooterView.setBackgroundDrawable(null);
				}
				ProgressBar progress = (ProgressBar) mFooterView.findViewById(R.id.pull_to_refresh_load_progress);
				TextView text = (TextView) mFooterView.findViewById(R.id.pull_to_refresh_loadmore_text);
				switch (getState()) {
				case STATE_LOAD_MORE:
